    ...
    if (!isLoggedIn) {
        return (
            <div className="flex flex-col items-center justify-center min-h-[60vh] animate-in fade-in duration-700">
                <div className="bg-[#1C1612]/60 backdrop-blur-3xl rounded-[2.5rem] p-12 shadow-2xl border border-[#2D241E] w-full max-w-md">
                    ...
                    <h2 className="text-3xl font-heading font-black text-white text-center mb-8 tracking-tighter italic">ADMIN <span className="text-red-500">LOGIN</span></h2>
                    ...
                    <form onSubmit={handleLogin} className="space-y-6">
                        ...
                        <div className="space-y-4">
                            <input 
                                ...
                                className="w-full bg-[#2D241E] border-none rounded-2xl px-6 py-4 text-xs font-bold text-white outline-none focus:ring-2 focus:ring-red-500/50 shadow-inner placeholder:text-gray-600 uppercase tracking-widest"
                            />
                            <input 
                                type="password"
                        ...
                        <button
                            type="submit"
                            className="w-full py-5 bg-gradient-to-r from-red-600 to-orange-600 text-white font-heading font-black rounded-full shadow-2xl shadow-red-900/40 hover:brightness-110 active:scale-95 transition-all text-sm uppercase tracking-[0.3em] outline-none"
                        >
                            Access Dashboard
                        </button>
                    </form>
        ...
        <div className="animate-in fade-in slide-in-from-bottom-5 duration-700">
            <div className="flex flex-col md:flex-row gap-8">
                ...
                <div className="md:w-64 flex-shrink-0">
                    <div className="sticky top-28 bg-[#1C1612]/60 backdrop-blur-3xl rounded-[2rem] p-6 border border-[#2D241E] shadow-2xl">
                        <div className="mb-10">
                            <h2 className="text-2xl font-heading font-black text-white tracking-tighter italic">
                        ...
                        <nav className="space-y-2 mb-10">
                            <button
                                ...
                                className={`w-full flex items-center gap-3 px-5 py-4 rounded-2xl text-xs font-heading font-black uppercase tracking-widest transition-all ${
                        ...
                        <button 
                            ...
                            className="w-full px-6 py-4 bg-[#2D241E] text-white font-heading font-black text-[10px] uppercase tracking-widest rounded-2xl hover:bg-gray-700 hover:text-red-400 border border-[#3D3128] hover:border-red-500/50 transition-all flex items-center justify-center gap-2"
                        >
                ...
                <div className="flex-grow animate-in fade-in slide-in-from-right-5 duration-500 pb-20">
                    ...
                    {activeView === 'overview' && (
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            <div className="bg-[#1C1612]/60 backdrop-blur-3xl rounded-[2rem] p-8 border border-[#2D241E] shadow-xl">
                                ...
                                <span className="text-4xl font-heading font-black text-white">{histories.filter(h => h.type === 'recipe').length || 8492}</span>
                            </div>
                            <div className="bg-[#1C1612]/60 backdrop-blur-3xl rounded-[2rem] p-8 border border-[#2D241E] shadow-xl">
                                <div className="flex items-center justify-between mb-4">
                                    <div className="w-12 h-12 bg-amber-900/30 text-amber-500 rounded-xl flex items-center justify-center">
                                        <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 4v1m6 11h2m-6 0h-2v4m0-11v3m0 0h.01M12 12h4.01M16 20h4M4 12h4m12 0h.01M5 8h2a1 1 0 001-1V5a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1zm12 0h2a1 1 0 001-1V5a1 1 0 00-1-1h-2a1 1 0 00-1 1v2a1 1 0 001 1zM5 20h2a1 1 0 001-1v-2a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1z" /></svg>
                                    </div>
                                    <span className="text-amber-500 font-bold text-sm">Active</span>
                                </div>
                                <h4 className="text-gray-500 text-xs font-bold uppercase tracking-widest mb-1">API Node Status</h4>
                                <span className="text-4xl font-heading font-black text-white">Healthy</span>
                            </div>
                            <div className="bg-[#1C1612]/60 backdrop-blur-3xl rounded-[2rem] p-8 border border-[#2D241E] shadow-xl">
                                <div className="flex items-center justify-between mb-4">
                                ...
                                <span className="text-4xl font-heading font-black text-white">{users.length > 0 ? users.length : 1204}</span>
                            </div>
                        </div>
                    )}

                    {activeView === 'users' && (
                        <div className="bg-[#0D0B08]/50 rounded-3xl border border-[#2D241E] overflow-hidden shadow-2xl">
                            <div className="bg-[#1C1612]/80 px-8 py-6 border-b border-[#2D241E] flex flex-col md:flex-row md:items-center justify-between gap-4">
                                ...
                                <div className="flex items-center gap-2">
                                    <input type="text" placeholder="Search ID or Email..." className="bg-[#0D0B08]/50 border border-[#3D3128] rounded-xl px-4 py-2 text-xs font-bold text-white outline-none w-48" />
                            ...
                            <div className="p-0 overflow-x-auto">
                                <table className="w-full text-left text-sm text-gray-400 min-w-[600px]">
                                    <thead className="bg-[#1C1612]/30 text-[10px] uppercase font-heading font-black tracking-widest text-gray-500 border-b border-[#2D241E]">
                                        <tr>
                                    ...
                                    <tbody className="divide-y divide-gray-800/50">
                                        {users.length > 0 ? users.map((user, i) => (
                                            <tr key={i} className="hover:bg-[#2D241E]/40 transition-colors group">
                                                <td className="px-8 py-5 font-mono text-xs opacity-50 group-hover:text-amber-400 group-hover:opacity-100 transition-colors">{user.uid || user.id || `usr_abc${i}xyz`}</td>
                                                <td className="px-8 py-5 font-bold text-gray-200">{user.email || user.displayName || 'Guest User'}</td>
                                                <td className="px-8 py-5 text-center">
                                                    <span className={`inline-block text-[10px] font-heading font-black uppercase tracking-widest px-4 py-1.5 rounded-full border ${user.role === 'admin' ? 'bg-red-500/10 text-red-500 border-red-500/30 shadow-[0_0_10px_rgba(239,68,68,0.2)]' : 'bg-blue-500/10 text-blue-400 border-blue-500/20'}`}>
                                                        {user.role || 'user'}
                                                    </span>
                                                </td>
                    ...
                    {activeView === 'history' && (
                        <div className="bg-[#0D0B08]/50 rounded-3xl border border-[#2D241E] overflow-hidden shadow-2xl">
                            <div className="bg-[#1C1612]/80 px-8 py-6 border-b border-[#2D241E] flex items-center justify-between">
                                <h3 className="text-sm font-bold text-white uppercase tracking-widest flex items-center gap-3">
                                ...
                                <span className="text-xs font-bold text-amber-500 bg-amber-500/10 px-4 py-2 flex items-center gap-2 rounded-xl border border-amber-500/20">
                                        ...
                                        {histories.length > 0 ? histories.map((history, i) => (
                                            <tr key={i} className="hover:bg-[#2D241E]/40 transition-colors group">
                                                ...
                                                <td className="px-8 py-5">
                                                    <span className={`inline-block text-[10px] font-heading font-black uppercase tracking-widest px-4 py-1.5 rounded-full border ${history.type === 'recipe' ? 'bg-orange-500/10 text-orange-500 border-orange-500/20' : 'bg-blue-500/10 text-blue-500 border-blue-500/20'}`}>
                                                        {history.type || 'unknown'}
                                                    </span>
                                                </td>
                                                <td className="px-8 py-5 font-bold text-gray-200 group-hover:text-amber-400 transition-colors">{history.title || 'Untitled Generation'}</td>
                    ...
                    {activeView === 'logs' && (
                        <div className="bg-[#0D0B08]/50 rounded-3xl border border-[#2D241E] overflow-hidden shadow-2xl">
                            <div className="bg-[#1C1612]/80 px-8 py-5 border-b border-[#2D241E] flex items-center justify-between">
                                <h3 className="text-sm font-bold text-white uppercase tracking-widest flex items-center gap-3">
                            ...
                            <div className="p-8 font-mono text-xs text-gray-400 space-y-4 min-h-[400px]">
                                ...
                                <div className="flex space-x-4"><span className="text-amber-500">[DB]</span><span>{new Date().toISOString().split('T')[0]} 10:15:05 - Cache refreshed automatically.</span></div>
